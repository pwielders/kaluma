...
typedef void (*socket_received)  (const uint8_t fd, const uint16_t length, const uint8_t* buffer, const struct ip_addr_t* address, const uint16_t port);
...
int  wifi_reset();
int  wifi_access_point(const char* ssid, const char* passwd, const struct ip_addr_t* gateway, const uint8_t mask);
...
typedef void (*wifi_link)(const char* ssid, const bool connected);
